...
| planning | repo reads, optional `explorer` checks, root `request_user_input`, native goal tools | exploration evidence, scope, task order, ownership, verification, goal state or manual `/goal` launch prompt | implementation plan and goal-backed launch are decision-complete | execution |
