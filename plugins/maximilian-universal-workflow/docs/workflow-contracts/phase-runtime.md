...
| planning | repo reads, optional `explorer` checks, root `request_user_input`, goal tools when useful | exploration evidence, scope, task order, ownership, verification, `/goal` launch prompt | implementation plan and goal-backed launch are decision-complete | execution |
