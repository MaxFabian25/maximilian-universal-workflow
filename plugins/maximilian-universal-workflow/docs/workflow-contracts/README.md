...
- Goal-backed planning: `planning` outputs the plan first, then uses native goal tools by default. Use `get_goal` to compare current goal state against the planned objective. Use `create_goal` when no current goal exists and the plan is decision-complete. If a different current goal exists, call `request_user_input` for the goal conflict choice. A normal workflow invocation is sufficient goal setup intent.
