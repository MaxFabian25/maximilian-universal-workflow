...
Use `planning` for repo plans and default native goal-backed execution setup. Planning inspects active goal state, resolves active-goal conflicts with `request_user_input`, and creates the default goal when no current goal exists after the plan is decision-complete. Normal workflow invocations use this goal-backed setup unless the user explicitly asks for planning-only, no-goal, or stop-with-evidence behavior.
