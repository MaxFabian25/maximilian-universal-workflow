...
Read `../../docs/workflow-contracts/phase-core.md` and `../../docs/workflow-contracts/evidence-discipline.md`. Use other workflow-contract files only when the core requires deeper authority.
...
- For unexpected failures or bad outputs, build or preserve the named proof surface before fixing; test one hypothesis at a time.
- Keep changed-path ownership explicit and non-overlapping.
- Use Codex-native subagents for non-overlapping implementation, artifact work, or independent checks when ownership is clear and integration stays in the running thread.
