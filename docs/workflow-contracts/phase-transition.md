...
- record diagnosis-loop status for unexpected results;
- record information-structure changes, vocabulary or decision capture, and shaped work items when relevant;
...
- Apply `evidence-discipline.md` before mutation and before completion claims; use `proof-first-change.md` for material mutation and `diagnosis-loop.md` for unexpected results. Scratch work and subagent results must have an owner and disposition before `done`.
- Apply `information-structure.md` when files or folders move, split, merge, or change role. Apply `work-item-shaping.md` when work is packaged for another owner.
