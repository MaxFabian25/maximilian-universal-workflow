...
- exploration/review: local reads first; cite files, commands, branch status, and current evidence.
...
- execution: require the `git-worktrees` branch-safety handoff before write-owning mutation; keep mutable scope explicit and non-overlapping. Goal bundles are binding execution context. Missing goal state is a planning/setup gap because default goal-backed planning should have created or preserved native goal state before execution.
- verification: use current-state commands/checklists; summaries and old output are not proof; complete active goals only after `get_goal` identity check and proof.
